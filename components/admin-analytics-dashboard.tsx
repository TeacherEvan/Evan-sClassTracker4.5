...
export function AdminAnalyticsDashboard({ userId, currentUser: _currentUser }: AdminAnalyticsDashboardProps) {
    ...
    const [_selectedSchoolId, _setSelectedSchoolId] = useState<Id<"schools"> | "all">("all");
    ...
    const stats = useMemo(() => {
        ...
        const recentClasses = allClasses.filter(c =>
            c.scheduledDate && c.scheduledDate >= thirtyDaysAgo
        );

        // Classes by status
        const approvedClasses = recentClasses.filter(c => c.status === "approved");
        ...
        const completedClasses = approvedClasses.filter(c => c.scheduledDate < Date.now());
        ...
        // Classes by school
        const classesBySchool = activeSchools.map(school => {
            const schoolClasses = recentClasses.filter(c => c.schoolId === school._id);
            const schoolApproved = schoolClasses.filter(c => c.status === "approved");
            return {
                school,
                totalClasses: schoolClasses.length,
                approvedClasses: schoolApproved.length,
                pendingClasses: schoolClasses.filter(c => c.status === "pending").length,
                completedClasses: schoolApproved.filter(c => c.scheduledDate < Date.now()).length,
            };
    ...
    const handleExportCSV = () => {
        ...
        const rows = stats.classesBySchool.map((item) => [
            language === "en" ? item.school.name : (item.school.nameTh || item.school.name),
            item.totalClasses,
        ...
        <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        {t("Admin Analytics Dashboard", "แดชบอร์ดการวิเคราะห์สำหรับผู้ดูแลระบบ")}
                    </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.approvedClasses}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("classes", "คลาส")}</p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.pendingClasses}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("classes", "คลาส")}</p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.completedClasses}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("classes", "คลาส")}</p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        {t("Classes by School (Last 30 Days)", "คลาสตามโรงเรียน (30 วันที่ผ่านมา)")}
                    </h2>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {stats.classesBySchool.length === 0 ? (
                                ...
                                stats.classesBySchool.map((item) => (
                                    <tr key={item.school._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
