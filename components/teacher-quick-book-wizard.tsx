...
export function TeacherQuickBookWizard({
  ...
  const bookingTemplates = useMemo(() => {
    ...
    for (const cls of recentApprovedClasses) {
      ...
      if (
        !templateMap.has(key) ||
        cls.scheduledDate > templateMap.get(key)!.lastBooked
      ) {
        templateMap.set(key, {
    ...
    return Array.from(templateMap.values())
      .sort((a, b) => b.lastBooked - a.lastBooked)
      .slice(0, 10);
            ...
            {bookingTemplates.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm mt-2">
                  {t(
                    "Book some classes first to use quick rebook",
                    "จองคลาสก่อนเพื่อใช้การจองด่วน",
                  )}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                          {t("Last booked:", "จองล่าสุด:")}{" "}
                          {new Date(template.lastBooked).toLocaleDateString()}
                        </div>
                      </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm">
              {t(
                "Select dates for this booking pattern",
                "เลือกวันที่สำหรับรูปแบบการจองนี้",
              )}
            </p>
            ...
            {selectedDates.length > 0 && (
              <div className="mt-4">
                ...
                >
                  {t(
                    `Continue (${selectedDates.length} dates)`,
                    `ดำเนินการต่อ (${selectedDates.length} วัน)`,
                  )}
                </button>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
              ...
              <div className="text-sm text-green-800 dark:text-green-200 space-y-1">
                ...
                <div>
                  <strong>{t("School:", "โรงเรียน:")}</strong>{" "}
                  {selectedTemplate?.schoolName}
                </div>
                <div>
                  <strong>{t("Time:", "เวลา:")}</strong>{" "}
                  {selectedTemplate?.time}
                </div>
                <div>
                  <strong>{t("Dates:", "วันที่:")}</strong>{" "}
  ...
  const progress =
    currentStep === "templates" ? 33 : currentStep === "calendar" ? 66 : 100;
