...
export function ModeratorApprovalWizard({
  ...
  const [selectedClassIds, setSelectedClassIds] = useState<Set<Id<"classes">>>(
    new Set(),
  );
  const [rejectionNotes, setRejectionNotes] = useState<
    Map<Id<"classes">, string>
  >(new Map());
  ...
  const pendingClassDetails = useMemo(() => {
    if (!pendingClasses || !allStudents || !allTeachers || !allSchools)
      return [];
  ...
  const filteredClasses = useMemo(() => {
    ...
    return pendingClassDetails.filter(
      (cls) => cls.teacherId === filterTeacherId,
    );
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      const approvalPromises = Array.from(selectedClassIds).map(
        async (classId) => {
          try {
      ...
      toast.error(
        "Failed to approve some classes",
        "การอนุมัติบางคลาสไม่สำเร็จ",
      );
    } finally {
  ...
  const handleBulkReject = async () => {
    ...
    try {
      const rejectionPromises = Array.from(selectedClassIds).map(
        async (classId) => {
          try {
            const notes =
              rejectionNotes.get(classId) || "Rejected by moderator";
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                    ...
                    {t("classes waiting for approval", "คลาสรอการอนุมัติ")}
                  </p>
                </div>
            ...
            {uniqueTeachers.length > 1 && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFilterTeacherId(e.target.value as Id<"users"> | "")
                  }
            ...
            {filteredClasses.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>
                  {t("No pending approvals", "ไม่มีการอนุมัติที่รอดำเนินการ")}
                </p>
                <p className="text-sm mt-2">
                  {t(
                    "All caught up! Great job! 🎉",
                    "ทำงานเสร็จหมดแล้ว! เยี่ยมมาก! 🎉",
                  )}
                </p>
              </div>
            ) : (
                    ...
                    {selectedClassIds.has(cls._id) && (
                      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                        <input
                          ...
                          placeholder={t(
                            "Add reason if rejecting (optional)",
                            "เพิ่มเหตุผลหากปฏิเสธ (ไม่บังคับ)",
                          )}
              ...
              <div className="sticky bottom-0 bg-white dark:bg-gray-800 pt-4 border-t border-gray-200 dark:border-gray-700 -mx-6 px-6 -mb-6 pb-6">
                <div className="flex gap-3">
                  ...
                  >
                    <Check className="w-5 h-5" />
                    {t(
                      `Approve ${selectedClassIds.size}`,
                      `อนุมัติ ${selectedClassIds.size}`,
                    )}
                  </button>
                  <button
                  ...
                  >
                    <X className="w-5 h-5" />
                    {t(
                      `Reject ${selectedClassIds.size}`,
                      `ปฏิเสธ ${selectedClassIds.size}`,
                    )}
                  </button>
                </div>
              </div>
              ...
              <div className="text-gray-600 dark:text-gray-400 space-y-2">
                {approvedCount > 0 && (
                  <p className="text-green-600 dark:text-green-400">
                    ✓ {approvedCount}{" "}
                    {t("class(es) approved", "คลาสได้รับการอนุมัติ")}
                  </p>
                )}
                {rejectedCount > 0 && (
