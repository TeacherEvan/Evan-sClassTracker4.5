...
export function TeacherConnectionManager({
  currentUser,
}: TeacherConnectionManagerProps) {
  const { t } = useLanguage();
  ...
  const connectTeacher = async (_args: any) => {
    throw new Error(
      "Feature not yet implemented - api.teacherSchools not exported from Convex",
    );
  };
  ...
  const connectedTeacherIds = useMemo(() => {
    return new Set(
      (connectedTeachers || []).map(
        (ct: { teacherId: string }) => ct.teacherId,
      ),
    );
  }, [connectedTeachers]);
  ...
  if (true) {
    // Always return early until teacherSchools API is exported
    return (
  ...
  const handleConnect = async () => {
    if (!selectedTeacherId || !selectedSchoolId) {
      toast.error("Please select a teacher", "กรุณาเลือกครู");
      return;
    }
    ...
    try {
      ...
      toast.success("Teacher connected successfully", "เชื่อมต่อครูสำเร็จ");
  ...
  const handleDisconnect = async (
    connectionId: Id<"teacherSchoolConnections">,
  ) => {
  ...
  const confirmDisconnect = async (
    ...
    try {
      ...
      toast.success(
        ...
        "ยกเลิกการเชื่อมต่อครูเรียบร้อยแล้ว",
      );
      setDisconnectingId(null);
    } catch (error) {
      console.error("Failed to disconnect teacher:", error);
      toast.error(
        "Failed to disconnect teacher",
        "ไม่สามารถยกเลิกการเชื่อมต่อครูได้",
      );
    }
      ...
      {selectedSchoolId ? (
                      ...
                      {disconnectingId === ct.connectionId ? (
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() =>
                              confirmDisconnect(
                                ct.connectionId as Id<"teacherSchoolConnections">,
                              )
                            }
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-8 text-center">
          <p className="text-gray-600">
            {t(
              "Please select a school to view and manage teacher connections",
              "กรุณาเลือกโรงเรียนเพื่อดูและจัดการการเชื่อมต่อครู",
            )}
          </p>
        </div>
      ) : null}
