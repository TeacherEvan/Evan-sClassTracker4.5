...
export function HierarchicalStudentSelector({
  ...
  const baseSelectClass =
    "w-full px-4 py-3 md:py-2 text-base md:text-sm border border-gray-300 rounded-xl md:rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 touch-manipulation transition-all";
      ...
      {students && students.some((s) => s.guardianName) && (
        <div>
          <label className="block text-xs font-medium mb-1.5 text-purple-600 dark:text-purple-400">
            {t(
              "Guardian Students (Auto-approved)",
              "นักเรียนของผู้ปกครอง (อนุมัติอัตโนมัติ)",
            )}
          </label>
          <select
            value={value}
