...
export function SchoolManagement({ currentUser }: SchoolManagementProps) {
    ...
    const [editingSchool, setEditingSchool] = useState<Id<"schools"> | null>(null);
    ...
    const handleEdit = (school: { _id: Id<"schools">; name: string; nameTh: string; moderatorId?: Id<"users"> }) => {
            ...
            {showForm && (
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label htmlFor="moderator" className="block text-sm font-medium mb-2">
                                {t("Assign Moderator (Optional)", "มอบหมายผู้ดูแล (ไม่บังคับ)")}
                            </label>
                            <select
                                id="moderator"
                                value={moderatorId}
                                onChange={(e) => setModeratorId(e.target.value as Id<"users"> | "")}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700"
                            >
                                <option value="">
                            ...
                            {(!schools || schools.length === 0) && (
                                <tr>
                                    <td colSpan={3} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                        {t("No schools found. Create one to get started!", "ไม่พบโรงเรียน สร้างโรงเรียนเพื่อเริ่มต้น!")}
                                    </td>
                                </tr>
                            )}
                        </tbody>
