...
export function SangsomStudentImportButton() {
  const { t } = useLanguage();
  const findSangsomSchool = useMutation(
    api.importSangsomStudents.findSangsomSchool,
  );
  const importK19Students = useMutation(
    api.importSangsomStudents.importK19Students,
  );
  const importK26Students = useMutation(
    api.importSangsomStudents.importK26Students,
  );
  const importK28Students = useMutation(
    api.importSangsomStudentsExtra.importK28Students,
  );
  const importK16Students = useMutation(
    api.importSangsomStudentsExtra.importK16Students,
  );
  const importK15Students = useMutation(
    api.importSangsomStudentsExtra.importK15Students,
  );
  const importK11Students = useMutation(
    api.importSangsomStudentsExtra.importK11Students,
  );
  ...
  const handleCheckSchool = async () => {
    ...
    try {
      ...
      if (data.found) {
        ...
        toast.error(
          data.message ||
            "Sangsom Kindergarten not found. Please seed Sangsom data first.",
      ...
      <div className="flex gap-3 mb-6">
        ...
        >
          {loading
            ...
            : t("Check School", "ตรวจสอบโรงเรียน")}
        </button>
      </div>
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            ...
            <div className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
              <p>
                ✅ {t("Imported", "นำเข้าสำเร็จ")}: {result.imported}
              </p>
              <p>
                ❌ {t("Failed", "ไม่สำเร็จ")}: {result.failed}
              </p>
            </div>
          </div>
          ...
          {result.results.length > 0 && (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              <p className="font-medium text-green-900 dark:text-green-100 mb-2">
                📋 {t("Imported Students", "นักเรียนที่นำเข้า")} (
              ...
              <div className="max-h-48 overflow-y-auto">
                <ul className="text-sm text-green-800 dark:text-green-200 space-y-1">
                  {result.results.map((student, idx) => (
                    <li key={idx}>
                      {idx + 1}. {student.nickname} ({student.thaiName}) -{" "}
                      {student.studentId}
                    </li>
                  ))}
                </ul>
