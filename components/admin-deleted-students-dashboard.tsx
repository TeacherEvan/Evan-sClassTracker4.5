...
export function AdminDeletedStudentsDashboard({
    ...
    const filteredDeletions = deletedStudents?.filter(log =>
    ...
    const filteredOrphaned = orphanedClasses?.filter(orphan =>
        orphan.deletionInfo?.studentName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        orphan.deletionInfo?.deletedBy?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        orphan.deletedStudentId?.toLowerCase().includes(searchTerm.toLowerCase())
    ) || [];
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl max-w-6xl w-full flex flex-col max-h-[85vh] shadow-2xl">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-white">
                                    {t("🚨 Deleted Students Investigation", "🚨 การสอบสวนนักเรียนที่ถูกลบ")}
                                </h2>
                                <p className="text-sm text-white/90">
                                    {t("Track WHO deleted WHAT student WHEN and WHY", "ติดตามว่าใครลบนักเรียนคนไหน เมื่อไหร่ และทำไม")}
                                </p>
                            </div>
                        </div>
                        <button
                ...
                <div className="flex border-b border-gray-200 dark:border-gray-700">
                    <button
                        ...
                        className={`flex-1 px-4 py-3 font-medium transition-colors ${activeTab === "deleted"
                            ? "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-b-2 border-red-600"
                            : "text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                            }`}
                    >
                        <Trash2 className="inline w-4 h-4 mr-2" />
                        {t("Deleted Students", "นักเรียนที่ถูกลบ")} ({filteredDeletions.length})
                    </button>
                    <button
                        onClick={() => setActiveTab("orphaned")}
                        className={`flex-1 px-4 py-3 font-medium transition-colors ${activeTab === "orphaned"
                            ...
                            {filteredDeletions.length === 0 ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400">
                                        {searchTerm
                                            ? t("No deleted students match your search", "ไม่พบนักเรียนที่ถูกลบตามการค้นหา")
                                            : t("No students have been deleted yet", "ยังไม่มีนักเรียนที่ถูกลบ")}
                                    </p>
                                </div>
                            ) : (
                                                ...
                                                {log.affectedClasses > 0 && (
                                                    <div className="mt-2 flex items-center gap-2 text-sm bg-orange-100 dark:bg-orange-900/20 px-3 py-1 rounded-md">
                                                        ...
                                                        <span className="text-orange-700 dark:text-orange-400">
                                                            {log.affectedClasses} {t("classes affected", "คลาสที่ได้รับผลกระทบ")}
                                                        </span>
                                                    </div>
                                                )}
                                            </div>
                        ...
                        <div className="space-y-3">
                            {filteredOrphaned.length === 0 ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400">
                                        {searchTerm
                                            ? t("No orphaned classes match your search", "ไม่พบคลาสที่เสียหายตามการค้นหา")
                                            : t("No orphaned classes found - system is clean!", "ไม่พบคลาสที่เสียหาย - ระบบสะอาด!")}
                                    </p>
                                </div>
                            ) : (
                                                ...
                                                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                                                    <div>
                                                        ...
                                                        <p className="font-medium">
                                                            {new Date(orphan.scheduledDate).toLocaleDateString(
                                                                language === "th" ? "th-TH" : "en-US"
                                                            )}
                                                        </p>
                                                    </div>
                                                    ...
                                                    <div className="bg-white dark:bg-gray-800 rounded-md p-3 border border-orange-200 dark:border-orange-700">
                                                        ...
                                                        <div className="space-y-1 text-sm">
                                                            ...
                                                            <p>
                                                                ...
                                                                <span className="font-medium">
                                                                    {new Date(orphan.deletionInfo.deletedAt).toLocaleString(
                                                                        language === "th" ? "th-TH" : "en-US"
                                                                    )}
                                                                </span>
                                                            </p>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full p-6">
                        ...
                        <div className="space-y-4">
                            <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
                                <h4 className="font-bold text-lg mb-2">{selectedDeletion.studentName}</h4>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <div>
                                    ...
                                    <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-3 space-y-2 text-sm">
                                        {selectedDeletion.parsedDetails.grade && (
                                            <p>
                                                <span className="font-medium">{t("Grade", "ชั้น")}:</span>{" "}
                                                {selectedDeletion.parsedDetails.grade}
                                            </p>
                                        )}
                                        {selectedDeletion.parsedDetails.class && (
                                            <p>
                                                <span className="font-medium">{t("Class", "ห้อง")}:</span>{" "}
                                                {selectedDeletion.parsedDetails.class}
                                            </p>
                                        )}
                                        {selectedDeletion.parsedDetails.affectedClasses !== undefined && (
                                            <p>
                                                <span className="font-medium">
