...
export function useClassBookingState(
  userId: Id<"users">,
  userRole: UserRole,
  ...
  const [classType, setClassType] = useState<
    "regular" | "makeup" | "trial" | "assessment"
  >("regular");

  // Student Creation State
  const [creatingStudent, setCreatingStudent] = useState(false);
  const [studentType, setStudentType] = useState<"school" | "provider">(
    "school",
  );
  ...
  const [pendingDeleteId, setPendingDeleteId] = useState<Id<"classes"> | null>(
    null,
  );
  const [showRejectDialog, setShowRejectDialog] = useState(false);
  const [pendingRejectId, setPendingRejectId] = useState<Id<"classes"> | null>(
    null,
  );
  const [rejectionReason, setRejectionReason] = useState("");

  // Filter States
  const [filterTeacherId, setFilterTeacherId] = useState<Id<"users"> | "all">(
    "all",
  );
  const [filterSchoolId, setFilterSchoolId] = useState<Id<"schools"> | "all">(
    "all",
  );
  ...
  const [expandedStudents, setExpandedStudents] = useState<Set<Id<"students">>>(
    new Set(),
  );
