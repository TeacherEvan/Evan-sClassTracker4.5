...
export function AdminContactButton({
  ...
  const generateUploadUrl = useMutation(
    api.adminContactRequests.generateUploadUrl,
  );
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast.error("File size exceeds 5MB limit", "ขนาดไฟล์เกิน 5MB");
      return;
    }
  ...
  const uploadFile = async (): Promise<{
    ...
    try {
      ...
      toast.error("Failed to upload file", "อัปโหลดไฟล์ล้มเหลว");
      return null;
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (!subject.trim() && !subjectTh.trim()) {
      toast.warning("Please provide a subject", "กรุณาระบุหัวเรื่อง");
      return;
    }
    ...
    try {
      ...
      await createContactRequest({
        ...
        message,
        messageTh,
        attachmentStorageId: attachmentData?.storageId as
          | Id<"_storage">
          | undefined,
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {(
                      ...
                      if (
                        type === "notification_window_request" &&
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col items-center gap-2 text-center">
                            <div
                              className={`${isSelected ? "text-orange-600 dark:text-orange-400" : "text-gray-600 dark:text-gray-400"}`}
                            >
                              {info.icon}
                            </div>
                            <span
                              className={`text-xs font-medium ${isSelected ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-gray-300"}`}
                            >
                              {info.title}
                            </span>
                          </div>
