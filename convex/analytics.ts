...
export const getSummaryAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.role === "teacher") {
      ...
      postClassNotes = noteResults.filter(
    ...
    const classesWithNotes = classes.filter((c) =>
      classIdsWithNotes.has(c._id),
    );
    ...
    const totalDuration = classes.reduce(
      ...
      .map((student, index) => {
        ...
        const avgClassCount =
          data.attendedClasses > 0
            ? Math.round(
                (data.totalDuration / 60 / data.attendedClasses) * 10,
              ) / 10 // Convert to ClassCount
            : 0;
