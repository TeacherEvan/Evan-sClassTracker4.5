...
// Create or get "Evan'sPVTclass" auto-generated provider for teacher self-reference
export const createOrGetEvansPVTClass = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Only teachers can have Evan's PVT Class providers
        if (teacher.role !== "teacher") {
            throw new Error("Only teachers can have Evan's PVT Class providers");
        }

        // Check if Evan's PVT Class provider already exists for this teacher
        ...
        if (existingProviders.length > 0) {
            ...
            return existingProviders[0]._id;
        }

        // Create new Evan's PVT Class provider
        const providerId = await ctx.db.insert("providers", {
...
// Get Evan's PVT Class provider for a teacher (query version)
export const getEvansPVTClass = query({
    args: {
        teacherId: v.id("users"),
    },
    handler: async (ctx, args) => {
