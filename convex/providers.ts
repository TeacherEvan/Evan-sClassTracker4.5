...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.role === "moderator") {
      throw new Error(
        "Moderators cannot create providers. Providers are for teachers and admins only.",
      );
    }
...
// Auto-create a personal provider for a teacher (e.g., "Evan'sPVTclass")
export const autoCreateTeacherProvider = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    // Only teachers can have auto-created providers
    if (teacher.role !== "teacher") {
      throw new Error("Only teachers can have auto-created personal providers");
    }

    // Check if teacher already has a personal provider with "PVTclass" pattern
    const existingProviders = await ctx.db
      .query("providers")
      .withIndex("by_created_by", (q) => q.eq("createdBy", args.teacherId))
      .filter((q) => q.eq(q.field("category"), "personal"))
      .collect();

    // Check if any existing provider matches the pattern (case-insensitive)
    const pvtClassProvider = existingProviders.find(
      (p) =>
        p.name.toLowerCase().includes("pvtclass") ||
        p.nameTh.toLowerCase().includes("pvtclass"),
    );

    if (pvtClassProvider) {
      // Provider already exists, return it
      return {
        providerId: pvtClassProvider._id,
        alreadyExists: true,
        message: `Provider "${pvtClassProvider.name}" already exists`,
      };
    }

    // Create new personal provider with teacher's name
    const teacherName = teacher.username || "Teacher";
    const providerName = `${teacherName}'sPVTclass`;
    const providerNameTh = `${teacherName}ของชั้นเรียนส่วนตัว`; // "Teacher's private class"

    const providerId = await ctx.db.insert("providers", {
      name: providerName,
      nameTh: providerNameTh,
      category: "personal",
...
// Get or create teacher's personal provider (idempotent)
export const getOrCreateTeacherProvider = mutation({
  args: {
    teacherId: v.id("users"),
  },
  handler: async (ctx, args) => {
    ...
    // Only teachers can have personal providers
    if (teacher.role !== "teacher") {
      throw new Error("Only teachers can have personal providers");
    }

    // Check if teacher already has a personal provider
    const existingProviders = await ctx.db
      .query("providers")
      .withIndex("by_created_by", (q) => q.eq("createdBy", args.teacherId))
      .filter((q) => q.eq(q.field("category"), "personal"))
    ...
    // Check for PVTclass pattern provider
    const pvtClassProvider = existingProviders.find(
      (p) =>
        p.name.toLowerCase().includes("pvtclass") ||
        p.nameTh.toLowerCase().includes("pvtclass") ||
    ...
    const teacherName = teacher.username || "Teacher";
    const providerName = `${teacherName}'sPVTclass`;
    const providerNameTh = `${teacherName}ของชั้นเรียนส่วนตัว`;
