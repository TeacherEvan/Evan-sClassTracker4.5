  ...
  handler: async (ctx, args) => {
    ...
    if (existingRequest) {
      const requestTypeText =
        existingRequest.requestType === "cancel"
          ? "cancellation"
          : "postponement";
      throw new Error(
        `A ${requestTypeText} request for this class is already pending`,
      );
    }
    ...
    if (classData.scheduledDate < Date.now()) {
      throw new Error(
        "Cannot cancel or postpone classes that have already passed",
      );
    }
    ...
    if (school && school.moderatorId && student) {
      const locationText =
        location?.name || classData.pendingLocationName || "Unknown location";
      const locationTextTh =
        location?.nameTh || classData.pendingLocationNameTh || "ไม่ทราบสถานที่";
...
export const approve = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (student) {
      ...
      const locationTextTh =
        location?.nameTh ||
        classData?.pendingLocationNameTh ||
        "ไม่ทราบสถานที่";
      ...
      const reviewNotesText = args.reviewNotes
        ? ` Notes: ${args.reviewNotes}`
        : "";
      const reviewNotesTextTh = args.reviewNotesTh
        ? ` หมายเหตุ: ${args.reviewNotesTh}`
        : "";
