...
function validateEventDates(eventDate: number, endDate?: number) {
  if (eventDate < Date.now() - 86400000) {
    // Allow past 24h for flexibility
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const finalSchoolId =
      args.visibility === "school" || args.visibility === "personal"
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.descriptionTh !== undefined)
      updates.descriptionTh = args.descriptionTh.trim();
    ...
    if (args.reminderMinutes !== undefined)
      updates.reminderMinutes = args.reminderMinutes;
    if (args.location !== undefined) updates.location = args.location.trim();
    if (args.locationTh !== undefined)
      updates.locationTh = args.locationTh.trim();
