...
export const countBcryptUsers = query({
  handler: async (ctx) => {
    ...
    const bcryptUsers = allUsers.filter((user) =>
      isBcryptHash(user.passwordHash),
    );
    const pbkdf2Users = allUsers.filter((user) =>
      user.passwordHash.startsWith("pbkdf2$"),
    );
    const btoaUsers = allUsers.filter(
      (user) =>
        !isBcryptHash(user.passwordHash) &&
        !user.passwordHash.startsWith("pbkdf2$"),
    );
...
export const resetAllBcryptPasswords = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!admin || admin.role !== "admin") {
      throw new Error(
        "❌ ADMIN ACCESS REQUIRED - Only admins can run password migrations",
      );
    }

    console.log(
      `🔐 Starting bcrypt password migration (dryRun: ${args.dryRun || false})`,
    );
    ...
    for (const user of bcryptUsers) {
      ...
      if (args.dryRun) {
        ...
        console.log(
          `✅ Migrated user: ${user.username} (${user.role}) - Language: ${user.preferredLanguage || "not set"}`,
        );
      }
    }
