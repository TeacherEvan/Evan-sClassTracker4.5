...
export const getTeacherClassCount = query({
  ...
  handler: async (ctx, args) => {
    ...
    const classIdsWithNotes = new Set(
      postClassNotes.map((note) => note.classId),
    );
    ...
    const classesWithNotes = classes.filter((cls) =>
      classIdsWithNotes.has(cls._id),
    );
...
export const getTeacherClassCountDetailed = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      throw new Error(
        "Unauthorized: Only moderators and admins can view detailed class counts",
      );
    }
    ...
    if (moderator.role === "moderator") {
      ...
      if (teacherClasses && teacherClasses.schoolId !== moderator.schoolId) {
        throw new Error(
          "Unauthorized: Moderators can only view teachers from their assigned school",
        );
      }
    }
    ...
    const totalClassCount = breakdown.reduce(
...
export const getMyClassCountDetails = query({
  ...
  handler: async (ctx, args) => {
    ...
    const schoolMap = new Map(
      schools.filter((s) => s !== null).map((s) => [s!._id, s]),
    );
    const providerMap = new Map(
      providers.filter((p) => p !== null).map((p) => [p!._id, p]),
    );
    const locationMap = new Map(
      locations.filter((l) => l !== null).map((l) => [l!._id, l]),
    );
    ...
    const classDetails = classesFiltered.map((cls) => {
      ...
      const bookedByUser = cls.bookedByUserId
        ? userMap.get(cls.bookedByUserId)
        : null;
      const approvedByUser = cls.approvedByUserId
        ? userMap.get(cls.approvedByUserId)
        : null;
      ...
      return {
        ...
        locationName:
          location?.name || cls.pendingLocationName || "Not specified",
        locationNameTh:
          location?.nameTh || cls.pendingLocationNameTh || "ไม่ระบุ",
        bookedByUserId: cls.bookedByUserId,
        bookedByUsername:
          cls.bookedByUsername || bookedByUser?.username || undefined,
        bookedByRole: bookedByUser?.role,
        approvedByUserId: cls.approvedByUserId,
        approvedByUsername:
          cls.approvedByUsername || approvedByUser?.username || undefined,
...
export const setTeacherCycle = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      teacher.schoolId !== moderator.schoolId
    ) {
      throw new Error(
        "Unauthorized: Moderators can only manage teachers from their school",
      );
    }
    ...
    const startDateStr = new Date(args.cycleStartDate).toLocaleDateString(
      "en-US",
      {
        year: "numeric",
        month: "short",
    ...
    const startDateStrTh = new Date(args.cycleStartDate).toLocaleDateString(
      "th-TH",
      {
        year: "numeric",
        month: "short",
    ...
    const endDateStrTh = new Date(args.cycleEndDate).toLocaleDateString(
      "th-TH",
      {
        year: "numeric",
        month: "short",
...
export const logClassCountView = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      throw new Error(
        "Unauthorized: Only moderators and admins can log class count views",
      );
    }
    ...
    const startDateStrTh = new Date(args.startDate).toLocaleDateString(
      "th-TH",
      {
        year: "numeric",
        month: "short",
...
export const getClassCountForPrint = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [students, schools, providers, locations] = await Promise.all([
      ...
      Promise.all(
        Array.from(schoolIds).map((id) => ctx.db.get(id as Id<"schools">)),
      ),
      Promise.all(
        Array.from(providerIds).map((id) => ctx.db.get(id as Id<"providers">)),
      ), // NEW: Fetch providers
      Promise.all(
        Array.from(locationIds).map((id) => ctx.db.get(id as Id<"locations">)),
      ),
    ]);

    const studentMap = new Map(
      students.filter((s) => s).map((s) => [s!._id, s!]),
    );
    const schoolMap = new Map(
      schools.filter((s) => s).map((s) => [s!._id, s!]),
    );
    const providerMap = new Map(
      providers.filter((p) => p).map((p) => [p!._id, p!]),
    ); // NEW: Provider map
    const locationMap = new Map(
      locations.filter((l) => l).map((l) => [l!._id, l!]),
    );
    ...
    const classDetails = countedClasses.map((cls) => {
      ...
      return {
        ...
        classCount: Math.round(classCount * 10) / 10,
        primaryStudentName: primaryStudent
          ? `${primaryStudent.firstName} ${primaryStudent.lastName}`.trim()
          : "Unknown",
        additionalStudentNames: additionalStudents.map((s) =>
          `${s.firstName} ${s.lastName}`.trim(),
        ),
