  ...
  handler: async (ctx, args) => {
    ...
    const performance = await Promise.all(
      Object.keys(teacherStats).map(async (teacherId) => {
        ...
        const approvalRate =
          processedClasses > 0 ? (stats.approved / processedClasses) * 100 : 0;
  ...
  handler: async (ctx, args) => {
    ...
    const averageClassesPerStudent =
      studentsWithClasses > 0
        ? Object.values(studentClassCounts).reduce(
            (sum, count) => sum + count,
            0,
          ) / studentsWithClasses
        : 0;
    ...
    return {
      ...
      averageClassesPerStudent:
        Math.round(averageClassesPerStudent * 100) / 100,
    };
  },
});
...
export const getLocationUtilization = query({
  ...
  handler: async (ctx, args) => {
    ...
    const locationStats: Record<
      string,
      { total: number; name: string; nameTh: string }
    > = {};
...
export const getEngagementMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const approvalRate =
      processedClasses > 0
        ? (statusCounts.approved / processedClasses) * 100
        : 0;
    const editRate =
      totalClasses > 0 ? (editedClassesCount / totalClasses) * 100 : 0;
    ...
    return {
      ...
      pendingResponseRate:
        totalClasses > 0
          ? Math.round((statusCounts.pending / totalClasses) * 100 * 100) / 100
          : 0,
    };
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    const currentWeekClasses = await ctx.db
      ...
      .withIndex("by_school_and_date", (q) =>
        q
          .eq("schoolId", args.schoolId)
          .gte("scheduledDate", currentWeekStart)
          .lte("scheduledDate", now),
      )
      .collect();
    ...
    const previousWeekClasses = await ctx.db
      ...
      .withIndex("by_school_and_date", (q) =>
        q
          .eq("schoolId", args.schoolId)
          .gte("scheduledDate", previousWeekStart)
          .lte("scheduledDate", currentWeekStart),
      )
      .collect();

    const currentWeekStats = {
      total: currentWeekClasses.length,
      approved: currentWeekClasses.filter((c) => c.status === "approved")
        .length,
      pending: currentWeekClasses.filter((c) => c.status === "pending").length,
      rejected: currentWeekClasses.filter((c) => c.status === "rejected")
        .length,
    };

    const previousWeekStats = {
      total: previousWeekClasses.length,
      approved: previousWeekClasses.filter((c) => c.status === "approved")
        .length,
      pending: previousWeekClasses.filter((c) => c.status === "pending").length,
      rejected: previousWeekClasses.filter((c) => c.status === "rejected")
        .length,
    };
    ...
    return {
      ...
      changes: {
        ...
        approved: calculateChange(
          currentWeekStats.approved,
          previousWeekStats.approved,
        ),
        pending: calculateChange(
          currentWeekStats.pending,
          previousWeekStats.pending,
        ),
        rejected: calculateChange(
          currentWeekStats.rejected,
          previousWeekStats.rejected,
        ),
      },
    };
  },
