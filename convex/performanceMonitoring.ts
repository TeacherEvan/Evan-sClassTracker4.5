...
export const getDetailedAuditLogs = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user || (user.role !== "admin" && user.role !== "moderator")) {
      throw new Error(
        "Unauthorized: Only admins and moderators can access detailed audit logs",
      );
    }
    ...
    if (args.filters?.targetUserId) {
      logs = await ctx.db
        .query("auditLogs")
        .withIndex("by_user", (q) =>
          q.eq("userId", args.filters!.targetUserId!),
        )
        .order("desc")
        .take(limit);
      ...
      logs = await ctx.db
        ...
        .withIndex("by_device_type", (q) =>
          q.eq("deviceType", args.filters!.deviceType!),
        )
        .order("desc")
        .take(limit);
      ...
      logs = await ctx.db
        .query("auditLogs")
        .withIndex("by_ip_address", (q) =>
          q.eq("ipAddress", args.filters!.ipAddress!),
        )
        .order("desc")
        .take(limit);
      ...
      logs = await ctx.db
        ...
        .withIndex("by_session", (q) =>
          q.eq("sessionId", args.filters!.sessionId!),
        )
        .order("desc")
        .take(limit);
    } else {
    ...
    if (args.filters?.startDate || args.filters?.endDate) {
      logs = logs.filter((log) => {
        if (args.filters!.startDate && log.timestamp < args.filters!.startDate)
          return false;
        if (args.filters!.endDate && log.timestamp > args.filters!.endDate)
          return false;
        return true;
      });
    }
...
export const getPerformanceStatistics = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!admin || admin.role !== "admin") {
      throw new Error(
        "Unauthorized: Only admins can access performance statistics",
      );
    }
...
export const getSessionTimeline = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!admin || (admin.role !== "admin" && admin.role !== "moderator")) {
      throw new Error(
        "Unauthorized: Only admins and moderators can access session timelines",
      );
    }
    ...
    const timeline = logs.map((log, index) => ({
      ...
      timeFromPrevious:
        index > 0 ? log.timestamp - logs[index - 1].timestamp : 0,
    }));
...
export const getSuspiciousActivity = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!admin || admin.role !== "admin") {
      throw new Error(
        "Unauthorized: Only admins can access suspicious activity alerts",
      );
    }
    ...
    const bulkDeletions = logs.filter(
      (l) =>
