...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (classData.teacherId !== args.teacherId) {
      throw new Error(
        "Unauthorized: You can only add notes to your own classes",
      );
    }
...
export const getByTeacher = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.startDate && args.endDate) {
      notes = notes.filter(
        (note) =>
          note.createdAt >= args.startDate! && note.createdAt <= args.endDate!,
      );
    }
