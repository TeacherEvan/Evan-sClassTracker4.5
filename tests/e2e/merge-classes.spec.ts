...
test.describe("Merge Classes Multi-Select Feature", () => {
  ...
  test("merge modal displays groups with mergeable classes", async ({
    page,
  }) => {
    // Click merge classes button (if it exists)
    const mergeButton = page
      .locator('button:has-text("Merge Classes"), button:has-text("รวมคลาส")')
      .first();
    ...
    if (await mergeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await page.waitForSelector('text="Merge Classes", text="รวมคลาส"', {
        timeout: 5000,
      });
      ...
      if (groupCount > 0) {
        ...
        await expect(
          page.locator('text="Location:", text="สถานที่:"'),
        ).toBeVisible();
        await expect(
          page.locator('text="Date/Time:", text="วัน/เวลา:"'),
        ).toBeVisible();
        await expect(
          page.locator('text="Total Classes:", text="จำนวนคลาส:"'),
        ).toBeVisible();
      } else {
        // Verify "no classes to merge" message
        await expect(
          page.locator("text=/No classes available to merge/i"),
        ).toBeVisible();
      }
    ...
    if (await mergeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (checkboxCount > 0) {
        ...
        if (isNowChecked) {
          ...
          await expect(
            page
              .locator('text="1. Select Target Class", text="1. เลือกคลาสหลัก"')
              .first(),
          ).toBeVisible();
        }
      }
      ...
      if (checkboxCount > 0) {
        ...
        if (radioCount > 0) {
          ...
          await expect(
            page
              .locator(
                'text="2. Select Classes to Merge", text="2. เลือกคลาสที่จะรวม"',
              )
              .first(),
          ).toBeVisible();
        }
    ...
    if (await mergeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (checkboxCount > 0) {
        ...
        if ((await targetRadios.count()) > 0) {
          ...
          await page.waitForSelector('text="2. Select Classes to Merge"', {
            timeout: 2000,
          });
  ...
  test("merge button is disabled when no groups are selected", async ({
    page,
  }) => {
    const mergeButton = page
      .locator('button:has-text("Merge Classes"), button:has-text("รวมคลาส")')
    ...
    if (await mergeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const submitButton = page
        .locator(
          'button:has-text("Merge Selected Groups"), button:has-text("รวมกลุ่มที่เลือก")',
        )
        .first();
  ...
  test("merge button is enabled when group has valid selection", async ({
    page,
  }) => {
    const mergeButton = page
      .locator('button:has-text("Merge Classes"), button:has-text("รวมคลาส")')
    ...
    if (await mergeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (checkboxCount > 0) {
        ...
        const firstGroup = page
          .locator('div[class*="border-2 rounded-xl"]')
          .first();
