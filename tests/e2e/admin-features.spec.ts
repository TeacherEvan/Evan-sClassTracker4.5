...
test.describe('Admin Features - Audit Trail', () => {
  ...
  test('TC-A4.2.1: Admin can access audit trail', async ({ page }) => {
    ...
    const hasAudit = await auditButton.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    if (hasAudit) {
      ...
      const auditHeader = page.locator(
        'h1:has-text("Audit"), h2:has-text("Audit"), h1:has-text("Logs"), h2:has-text("Logs")'
      ).first();

      const hasAuditInterface = await auditHeader.isVisible({ timeout: 3000 }).catch(() => false);

      if (hasAuditInterface) {
        console.log('✅ Audit trail interface is displayed');
      }
    } else {
      console.log('ℹ️ Audit trail button not found - may be in different location or navigation path');
    }
  });
  ...
  test('TC-A4.2.2: Audit trail displays user actions', async ({ page }) => {
    ...
    const auditTable = page.locator('table, [role="table"], [class*="audit"]').first();
    const hasTable = await auditTable.isVisible({ timeout: 3000 }).catch(() => false);

    if (hasTable) {
      // Count audit entries
      const auditRows = await page.locator('table tbody tr, [role="row"]').count();
      console.log(`✅ Found ${auditRows} audit log entries`);

      if (auditRows > 0) {
        // Check for expected columns
        const expectedColumns = [
          'User', 'Action', 'Timestamp', 'Target',
          'ผู้ใช้', 'การกระทำ', 'เวลา', 'เป้าหมาย'
        ];

        let foundColumns = 0;
        for (const col of expectedColumns) {
          const hasColumn = await page.locator(`th:has-text("${col}")`).isVisible({ timeout: 1000 }).catch(() => false);
          if (hasColumn) foundColumns++;
        }

        if (foundColumns > 0) {
          console.log(`✅ Audit table has ${foundColumns} expected column headers`);
        }
      }
    } else {
      console.log('ℹ️ Audit table not found - may require navigation to audit section');
    }
  });
  ...
  test('TC-A4.2.3: Audit log entries are immutable', async ({ page }) => {
    ...
    console.log('ℹ️ Backend should prevent modifications via database constraints');

    // Look for edit/delete buttons in audit interface
    const editButton = page.locator(
      'button:has-text("Edit"), button:has-text("แก้ไข")'
    ).first();

    const deleteButton = page.locator(
      'button:has-text("Delete"), button:has-text("ลบ")'
    ).first();
    ...
    if (!hasEdit && !hasDelete) {
      console.log('✅ No edit/delete buttons found in UI (expected for audit logs)');
    } else {
      console.log('⚠️ Edit/delete buttons found - verify these are not for audit log entries');
    }
  });
...
test.describe('Admin Features - Watchlist System', () => {
  ...
  test('TC-A4.1.1: Admin can access watchlist', async ({ page }) => {
    ...
    const watchlistButton = page.locator(
      'button:has-text("Watchlist"), button:has-text("Watch"), button:has-text("ติดตาม")'
    ).first();

    const hasWatchlist = await watchlistButton.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    if (hasWatchlist) {
      ...
      const watchlistHeader = page.locator(
        'h1:has-text("Watchlist"), h2:has-text("Watchlist")'
      ).first();

      const hasInterface = await watchlistHeader.isVisible({ timeout: 3000 }).catch(() => false);

      if (hasInterface) {
        console.log('✅ Watchlist interface is displayed');
      }
    } else {
      console.log('ℹ️ Watchlist feature not found - may not be implemented or requires different navigation');
    }
  });
  ...
  test('TC-A4.1.3: Watchlist alerts trigger for user actions', async () => {
    ...
    console.log('ℹ️ Expected: Admin receives notifications when watchlisted users perform actions');
...
test.describe('Admin Features - System Analytics', () => {
  ...
  test('admin can see system-wide analytics', async ({ page }) => {
    ...
    if (hasAnalytics) {
      ...
      const schoolFilter = page.locator(
        'select:has-text("School"), text="All Schools", text="ทุกโรงเรียน"'
      ).first();

      const hasSchoolFilter = await schoolFilter.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('admin can see all schools summary', async ({ page }) => {
    ...
    const schoolsButton = page.locator(
      'button:has-text("Schools"), button:has-text("โรงเรียน")'
    ).first();

    const hasSchools = await schoolsButton.isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('Data Migration - Guardian to Provider', () => {
  ...
  test('TC-D5.1.1: No legacy guardian users exist', async ({ page }) => {
    ...
    const usersButton = page.locator(
      'button:has-text("Users"), button:has-text("ผู้ใช้")'
    ).first();

    const hasUsers = await usersButton.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    if (hasUsers) {
      ...
      const guardianRole = page.locator(
        'text="Guardian", text="ผู้ปกครอง"'
      );

      const guardianCount = await guardianRole.count();

      if (guardianCount === 0) {
        console.log('✅ No "Guardian" role found in UI (expected - migration complete)');
      } else {
        console.log(`⚠️ Found ${guardianCount} references to "Guardian" role - may need cleanup`);
      }
    } else {
  ...
  test('TC-D5.1.2: Provider relationships are valid', async ({ page }) => {
    ...
    console.log('ℹ️ Expected: All guardian relationships migrated to providers table');
    console.log('ℹ️ Backend validation: providers.studentId and providers.providerId should reference valid records');
    console.log('ℹ️ UI validation: Students should show provider relationships, not legacy guardian field');

    // Navigate to student management
    const studentsButton = page.locator(
      'button:has-text("Students"), button:has-text("นักเรียน")'
    ).first();

    const hasStudents = await studentsButton.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    if (hasStudents) {
      ...
      if (hasProviderField > 0) {
        ...
        console.log('ℹ️ Provider field not prominently displayed - may be in student details');
      }
    }
  });
  ...
  test('TC-D5.1.3: UI removes guardian role references', async ({ page }) => {
    ...
    for (const section of sections) {
      ...
      const hasSection = await sectionButton.isVisible({ timeout: 2000 }).catch(() => false);
      ...
      if (hasSection) {
        ...
        const guardianReferences = await page.locator('text=/guardian/i').count();

        if (guardianReferences > 0) {
          console.log(`⚠️ Found ${guardianReferences} "guardian" references in ${section} section`);
        } else {
...
test.describe('Data Migration - Password Hashing', () => {
  ...
  test('TC-D5.2.1: All users migrated to PBKDF2', async ({ page }) => {
    ...
    console.log('ℹ️ Expected: All user password hashes use PBKDF2 format (not bcrypt)');
    console.log('ℹ️ Bcrypt hashes start with "$2" - should not exist after migration');
    console.log('ℹ️ PBKDF2 hashes use different format');
    console.log('ℹ️ Backend validation required - migration dashboard query exists in docs');
    ...
    const hasDashboard = await dashboardButton.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('TC-D5.2.2: Login works after migration', async ({ page }) => {
    ...
    const isLoggedIn = await userProfile.isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('Data Migration - Schema Cleanup', () => {
  test('TC-D5.3.1: No deprecated fields in active use', async () => {
    ...
    console.log('ℹ️ Backend validation: Review schema definition for outdated fields');
    console.log('ℹ️ UI validation: Ensure UI does not reference deprecated fields');
  ...
  test('TC-D5.3.2: All database indexes are utilized', async () => {
    ...
    console.log('ℹ️ Expected: All defined indexes in schema are used by queries');
    console.log('ℹ️ Backend validation: Grep codebase for .withIndex() calls');
    console.log('ℹ️ Performance validation: Queries should complete in <2 seconds');
  ...
  test('TC-D5.3.3: Data integrity validation', async ({ page }) => {
    ...
    console.log('ℹ️ Expected: No orphaned records, valid foreign keys, no NULL in required fields');
    ...
    const sections: ('Classes' | 'Students' | 'Users')[] = ['Classes', 'Students', 'Users'];
    ...
    for (const section of sections) {
      ...
      if (errorCount === 0) {
        ...
        console.log(`⚠️ Found ${errorCount} error messages in ${section} section`);
      }
    }
  });
});
...
test.describe('System Health - Performance', () => {
  ...
  test('page load times are under 2 seconds', async ({ page }) => {
    ...
    for (const section of sections) {
      ...
      if (loadTime < 2000) {
        ...
        console.log(`⚠️ ${section} exceeds performance SLA (${loadTime}ms > 2000ms)`);
      }
    }
  });
  ...
  test('mutations complete quickly', async () => {
    ...
    console.log('ℹ️ Expected: Create/Update/Delete operations complete in <1 second');
    console.log('ℹ️ Full test requires executing actual mutations and measuring time');
  });
});
