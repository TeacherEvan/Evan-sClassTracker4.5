...
test.describe("Student Management", () => {
  test("moderator can create new student", async ({ page }) => {
    ...
    await fillBilingualInput(
      page,
      "First Name",
      testData.firstName,
      testData.firstNameTh,
    );
    await fillBilingualInput(
      page,
      "Last Name",
    ...
    const gradeSelect = page
      ...
      .or(page.locator("select").filter({ hasText: "ระดับชั้น" }))
      .first();
    await gradeSelect.selectOption("K1");

    // Select school (if dropdown)
    const schoolSelect = page
      .locator("select")
      .filter({ hasText: "School" })
      .or(page.locator("select").filter({ hasText: "โรงเรียน" }))
      .first();
    if (await schoolSelect.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const classSelect = page
        ...
        .or(page.locator("select").filter({ hasText: "คลาส" }))
        .first();
    ...
    await page
      .locator(
        'button:has-text("Create"), button:has-text("สร้าง"), button[type="submit"]',
      )
      .first()
      .click();
    ...
    await expect(
      page.locator(`text=${testData.firstName}`).first(),
    ).toBeVisible({ timeout: 15000 });
  });

  test("student creation handles Thai characters correctly", async ({
    page,
  }) => {
    // Login as moderator
    await login(page, TEST_USERS.moderator);
    ...
    const thaiFirstNameInput = page
      .locator('input[placeholder*="ชื่อ"], input[aria-label*="ชื่อ"]')
      .first();
    const thaiLastNameInput = page
      .locator('input[placeholder*="นามสกุล"], input[aria-label*="นามสกุล"]')
      .first();

    if (
      await thaiFirstNameInput.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
      await thaiFirstNameInput.fill(thaiFirstName);
    }

    if (
      await thaiLastNameInput.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
      await thaiLastNameInput.fill(thaiLastName);
    }

    // Fill English equivalent
    const engFirstNameInput = page
      .locator('input[placeholder*="First"], input[aria-label*="First"]')
      .first();
    const engLastNameInput = page
      .locator('input[placeholder*="Last"], input[aria-label*="Last"]')
      .first();

    if (
      await engFirstNameInput.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
      await engFirstNameInput.fill("Somchai");
    }

    if (
      await engLastNameInput.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
    ...
    await expect(page.locator(`text=${thaiFirstName}`).first()).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("student ID is auto-generated", async ({ page }) => {
    ...
    const studentId = page
      .locator('[data-testid="student-id"], text=/^[A-Z0-9]{4}-[A-Z0-9]{4}-/')
      .first();
  ...
  test("duplicate student prevention", async ({ page }) => {
    ...
    const errorToast = page
      .locator(
        '[role="alert"], .toast-error, text=/duplicate|already exists|ซ้ำ/i',
      )
      .first();
  ...
  test("student search and filtering", async ({ page }) => {
    ...
    if (await searchInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const results = page.locator(
        '[data-testid="student-row"], [class*="student"]',
      );
