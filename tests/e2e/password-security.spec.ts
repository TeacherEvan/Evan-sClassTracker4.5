...
test.describe('Password Security', () => {
  ...
  test('should successfully login with PBKDF2 password hash', async ({ page }) => {
  ...
  test('should enforce minimum password length on password change', async ({ page }) => {
    // Login as teacher
    await login(page, TEST_USERS.teacher);
    ...
    const profileButton = page.locator('button:has-text("Profile"), button:has-text("โปรไฟล์")').first();
    if (await profileButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      await profileButton.click();
      
      // Try to change password with short password
      const changePasswordButton = page.locator('button:has-text("Change Password"), button:has-text("เปลี่ยนรหัสผ่าน")').first();
      if (await changePasswordButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        await changePasswordButton.click();
        
        // Fill in password change form with short password
        await page.locator('input[type="password"][placeholder*="Current"], input[type="password"][placeholder*="ปัจจุบัน"]').first().fill('TeacherEvan');
        await page.locator('input[type="password"][placeholder*="New"], input[type="password"][placeholder*="ใหม่"]').first().fill('short');
        
        // Submit
        await page.locator('button[type="submit"], button:has-text("Save"), button:has-text("บันทึก")').first().click();
  ...
  test('should handle account lockout after failed attempts', async ({ page }) => {
    ...
    // Fill credentials
    await page.locator('#username, input[type="text"]').first().fill('Evan');
    await page.locator('#password, input[type="password"]').first().fill('TeacherEvan');
    ...
    const calendarHeading = page.locator('h2:has-text("Monthly Calendar"), h2:has-text("ปฏิทินรายเดือน")').first();
    const lockoutMessage = page.locator('text=/Account locked|บัญชีถูกล็อก/i');
    
    // One of these should be visible
    await expect(calendarHeading.or(lockoutMessage)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should require password change for new users', async ({ page }) => {
    ...
    const passwordChangeDialog = page.locator('h2:has-text("Change Password"), h2:has-text("เปลี่ยนรหัสผ่าน")');
    const calendarHeading = page.locator('h2:has-text("Monthly Calendar"), h2:has-text("ปฏิทินรายเดือน")').first();
    
    // Either password change required OR already changed and calendar visible
    await expect(passwordChangeDialog.or(calendarHeading)).toBeVisible({ timeout: 10000 });
  });
