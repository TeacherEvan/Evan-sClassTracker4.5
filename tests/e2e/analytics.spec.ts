...
test.describe("Class Analytics", () => {
  test.describe("Teacher View", () => {
    ...
    test("teacher sees summary view by default", async ({ page }) => {
      ...
      const teacherComparisonTab = page
        ...
        .first();
      await expect(teacherComparisonTab)
  ...
  test.describe("Moderator View", () => {
    ...
    test("moderator can switch between Summary and Teacher Comparison tabs", async ({
      page,
    }) => {
      await login(page, TEST_USERS.moderator);
      await navigateToTab(page, "Classes");
      ...
      await expect(
        page
          .locator('th:has-text("Total Classes"), th:has-text("คลาสทั้งหมด")')
          .first(),
      ).toBeVisible({ timeout: 3000 });

      // Switch back to Summary tab
      const summaryTab = page
        .locator('button:has-text("Summary"), button:has-text("สรุป")')
        .first();
    ...
    test("Teacher Comparison shows teacher metrics", async ({ page }) => {
      ...
      for (const header of headers) {
        ...
        if (!isVisible) {
          console.log(
            `[TEST] Header "${header}" not found, checking Thai version`,
          );
        }
      }
    });
  ...
  test.describe("Admin View", () => {
    test("admin can access Teacher Comparison across all schools", async ({
      page,
    }) => {
      // Login as admin
  ...
  test.describe("Analytics Modal Interactions", () => {
    ...
    test("date range filtering works", async ({ page }) => {
      ...
      if (
        await startDateInput.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
