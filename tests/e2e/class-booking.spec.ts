...
test.describe("Class Booking Workflow", () => {
  test("teacher can book a class", async ({ page }) => {
    ...
    const schoolSelect = page
      .locator('select:has-option, [role="combobox"]')
      .first();
    ...
    await page
      .locator(
        'button:has-text("Book"), button:has-text("จอง"), button[type="submit"]',
      )
      .first()
      .click();
  ...
  test("moderator receives notification for new class booking", async ({
    page,
  }) => {
    ...
    const notificationBadge = page.locator(
      '[data-testid="notification-badge"], text=/\\d+/',
    );
  ...
  test("moderator can approve class booking", async ({
    page,
    convexClient,
  }) => {
    // 1. Seed data: Create a pending class
    const teachers = await convexClient.query(api.users.list, {
      role: "teacher",
    });
    const teacher = teachers[0];
    const schools = await convexClient.query(api.schools.list, {});
    const school = schools[0];
    const students = await convexClient.query(api.students.list, {
      schoolId: school._id,
    });
    ...
    const pendingClass = page
      .locator("text=pending, text=acknowledged, text=รอดำเนินการ")
      .first();
    ...
    if (await pendingClass.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (await rejectButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await reasonInput.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          await page
            .locator(
              'button:has-text("Confirm"), button:has-text("ยืนยัน"), button:has-text("Reject")',
            )
            .last()
            .click();
        }
  ...
  test("calendar view displays classes", async ({ page }) => {
    ...
    const calendarButton = page
      .locator(
        'button:has-text("Calendar"), button:has-text("ปฏิทิน"), [aria-label*="calendar"]',
      )
      .first();
    ...
    if (await calendarButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await expect(
        page.locator('[data-testid="calendar"], [class*="calendar"]').first(),
