...
test.describe('Lazy Loading', () => {
    test('bulk edit modal lazy loads correctly', async ({ page }) => {
        ...
        if (checkboxCount > 1) {
            ...
            if (await bulkEditButton.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    test('class analytics modal lazy loads correctly for moderator', async ({ page }) => {
        // Login as moderator
        await login(page, TEST_USERS.moderator);
        ...
        const detailsButton = page.locator('button:has-text("Details"), button:has-text("รายละเอียด"), button:has-text("View Details")').first();
    ...
    test('payment calculator modal lazy loads correctly', async ({ page }) => {
        ...
        const calculatorButton = page.locator('button:has([data-lucide="calculator"]), button:has-text("Calculator"), button:has-text("คำนวณ")').first();

        if (await calculatorButton.isVisible({ timeout: 2000 }).catch(() => false)) {
