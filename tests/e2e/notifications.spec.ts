...
test.describe("Notification System", () => {
  test("post-class notes notification to guardian", async ({ page }) => {
    ...
    const completedClass = page
      .locator("text=completed, text=เสร็จสิ้น")
      .first();
    ...
    if (await completedClass.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const notesButton = page
        .locator(
          'button:has-text("Notes"), button:has-text("บันทึก"), button:has-text("Post-Class")',
        )
        .first();
      ...
      if (await notesButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await notesTextarea.fill(
          "Test post-class notes for guardian notification",
        );

        // Submit
        await page
          .locator(
            'button:has-text("Send"), button:has-text("ส่ง"), button[type="submit"]',
          )
          .first()
          .click();
  ...
  test("notification window displays for new users", async ({ page }) => {
    ...
    const notificationWindow = page
      .locator(
        '[data-testid="notification-window"], [class*="notification-window"]',
      )
      .first();

    if (
      await notificationWindow.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
      ...
      const closeButton = page
        .locator(
          'button:has-text("Close"), button:has-text("ปิด"), [aria-label*="close"]',
        )
        .first();
  ...
  test("app update notification appears", async ({ page }) => {
    ...
    const updateNotification = page
      .locator(
        '[data-testid="app-update"], text=/Update|อัปเดต/, text=/Version|เวอร์ชัน/',
      )
      .first();

    if (
      await updateNotification.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
  ...
  test("real-time notification badge updates", async ({ page }) => {
    ...
    const notificationBadge = page
      .locator('[data-testid="notification-badge"], [class*="badge"]')
      .first();

    if (
      await notificationBadge.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
      // Get initial count
      ...
      await page.waitForSelector(
        '[data-testid="notification-panel"], [class*="notification"]',
        { timeout: 3000 },
      );
  ...
  test("notification preferences persist", async ({ page }) => {
    ...
    const settingsButton = page
      .locator(
        'button:has-text("Settings"), button:has-text("ตั้งค่า"), [aria-label*="settings"]',
      )
      .first();
    ...
    if (await settingsButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const notificationToggle = page
        .locator(
          'input[type="checkbox"][aria-label*="notification"], input[type="checkbox"][name*="notification"]',
        )
        .first();
      ...
      if (
        ...
        const saveButton = page
          .locator('button:has-text("Save"), button:has-text("บันทึก")')
          .first();
