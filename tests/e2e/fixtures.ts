...
export const test = base.extend<TestFixtures, WorkerFixtures>({
  ...
  workerAuthState: [
    async ({ browser }, run, workerInfo) => {
      ...
      console.log(
        `[Worker ${workerInfo.workerIndex}] Performing one-time login...`,
      );
      ...
      console.log(
        `[Worker ${workerInfo.workerIndex}] Auth state saved to ${authFile}`,
      );
  ...
  authenticatedPage: async (
    { browser, workerAuthState: _workerAuthState },
    run,
    workerInfo,
  ) => {
    const authFile = `.auth/worker-${workerInfo.workerIndex}.json`;
