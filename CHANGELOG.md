...
**Summary**: Comprehensive migration system to convert legacy guardian users and orphan students to the provider system with automated pseudo-provider creation.

### Added

#### Migration Scripts & Tools

- **convex/guardianToProviderMigration.ts** (Enhanced to 420 lines)
  - `previewMigration` query: Shows migration scope including orphan students
  - `migrateGuardiansToProviders` mutation: Converts guardians to providers + creates pseudo-provider
  - `rollbackMigration` mutation: Reverses migration safely
  - `cleanupDeprecatedFields` mutation: Removes deprecated fields in phases
  - `verifyCleanup` query: Shows migration completion status

- **scripts/run-guardian-migration.ps1** (325 lines)
  - Interactive PowerShell runner with safety checks
  - Pre-flight connection and backup verification
  - Dry-run and live migration modes
  - Cleanup phase support (students, classes, or all)
  - Post-migration verification helpers

- **scripts/test-guardian-migration.js** (140 lines)
  - Local testing helper for dev environments
  - Step-by-step testing instructions
  - Quick reference for Convex Dashboard operations

#### Documentation

- **docs/migrations/GUARDIAN_MIGRATION_RUNBOOK.md** (475 lines)
  - Complete step-by-step migration guide
  - Pre-migration checklist (tools, backups, previews)
  - Detailed migration procedures (dry-run → live → cleanup)
  - Verification and rollback instructions
  - Troubleshooting guide with 7 common issues + solutions
...
1. **Guardian User Conversion**
   - Creates provider entity for each guardian user
   - Category: "guardian"
   - Updates all linked students to use providerId
   - Updates all linked classes to use providerId

2. **Pseudo-Provider for Orphan Students**
   - Auto-creates `{Teacher'sPVTclass}` provider for students without links
   - Category: "personal" with `isAutoGenerated: true`
   - Handles students with no schoolId, providerId, or guardianId
   - Single shared provider for all orphan students

3. **Phased Cleanup**
   - Remove `guardianId` from students (phase: "students")
   - Remove `isGuardianLinked` from classes (phase: "classes")
   - Safety checks: Only removes if providerId exists
   - Can run all phases together (phase: "all")

4. **Safety & Rollback**
   - Dry-run mode to preview changes
   - Comprehensive logging at each step
   - Rollback capability to undo migration
   - Deletes auto-generated pseudo-provider on rollback

### Migration Execution Flow

```text
1. Preview → Check what will be migrated
2. Dry Run → Simulate migration (no changes)
3. Backup → Create database backup
4. Live Migration → Execute actual migration
5. Verification → Check completion status
6. Testing → Test application functionality
7. Cleanup → Remove deprecated fields (optional)
```

### Acceptance Criteria Met

- ✅ All guardian users can be converted to provider entities
- ✅ Orphan students get auto-generated pseudo-provider
- ✅ All data relationships maintained correctly
- ✅ Migration is reversible via rollback
- ✅ Deprecated fields can be safely removed
- ✅ Comprehensive documentation and runbook provided
- ✅ Testing tools for local development included

### Notes

- Guardian role remains in schema for data safety (can be removed after 90 days)
- Deprecated indexes remain for rollback safety
- Wait 48+ hours after migration before running cleanup
- Test thoroughly on dev/staging before production
