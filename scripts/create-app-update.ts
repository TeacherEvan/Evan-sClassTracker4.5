...
if (!CONVEX_URL) {
  ...
  console.error(
    '   $env:NEXT_PUBLIC_CONVEX_URL="your-convex-url"; npm run create-update',
  );
...
function parseImplementationSummaries(): UpdateData | null {
  ...
  if (summaryFiles.length === 0) {
    console.log(
      "⚠️  No IMPLEMENTATION_SUMMARY_*.md files found in project root",
    );
    console.log(
      "   Create one first with naming: IMPLEMENTATION_SUMMARY_[FEATURE]_[DATE].md",
    );
    return null;
  }
...
async function createAppUpdate(
  ...
  try {
    ...
    const existingUpdates = await client.query(api.appUpdates.list, {
      userId: adminUserId,
    });
    const duplicateUpdate = existingUpdates?.find(
      (u) => u.version === updateData.version,
    );

    if (duplicateUpdate) {
      console.log(
        `⚠️  Update version ${updateData.version} already exists (ID: ${duplicateUpdate._id})`,
      );
      ...
      console.log(
        `   2. Change the version number in your IMPLEMENTATION_SUMMARY filename`,
      );
      return null;
    }
