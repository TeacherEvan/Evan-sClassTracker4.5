...
const CONVEX_URL = process.env.NEXT_PUBLIC_CONVEX_URL || "https://resolute-basilisk-801.convex.cloud";
    ...
    try {
        ...
        if (users.length > 0) {
            ...
            if (sampleUser.createdAt) {
                ...
                console.log(`   createdAt: ${sampleUser.createdAt} → ${createdDate.toISOString()}`);
            ...
            if (sampleUser.lastSuccessfulLogin) {
                ...
                console.log(`   lastSuccessfulLogin: ${sampleUser.lastSuccessfulLogin} → ${loginDate.toISOString()}`);
        ...
        if (classes.length > 0) {
            ...
            if (sampleClass.scheduledDate) {
                ...
                console.log(`   scheduledDate: ${sampleClass.scheduledDate} → ${schedDate.toISOString()}`);
            ...
            if (sampleClass.createdAt) {
                ...
                console.log(`   createdAt: ${sampleClass.createdAt} → ${createdDate.toISOString()}`);
                console.log(`   Is valid date: ${!isNaN(createdDate.getTime())}`);
            }
        }
        ...
        if (students.length > 0) {
            ...
            if (sampleStudent.dateOfBirth) {
                ...
                console.log(`   dateOfBirth: ${sampleStudent.dateOfBirth} → ${dob.toISOString()}`);
