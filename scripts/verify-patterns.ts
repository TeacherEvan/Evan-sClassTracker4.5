...
function checkFile(filePath: string) {
    ...
    lines.forEach((line, index) => {
        ...
        if (line.match(/\balert\s*\(/)) {
            console.log(`${RED}ERROR: Found 'alert()' in ${relativePath}:${index + 1}${RESET}`);
            console.log(`  ${YELLOW}Rule: Use toast.success/error instead of alert()${RESET}`);
            errorCount++;
        }

        if (line.match(/\bconfirm\s*\(/)) {
            console.log(`${RED}ERROR: Found 'confirm()' in ${relativePath}:${index + 1}${RESET}`);
            console.log(`  ${YELLOW}Rule: Use custom modals or toast instead of confirm()${RESET}`);
            errorCount++;
        }
    });
    ...
    if (relativePath.startsWith('components') && fileName.endsWith('.tsx')) {
        if (!content.includes('"use client"') && !content.includes("'use client'")) {
            console.log(`${RED}ERROR: Missing "use client" directive in ${relativePath}${RESET}`);
            console.log(`  ${YELLOW}Rule: All UI components must be client-side${RESET}`);
            errorCount++;
        }
    }
}
