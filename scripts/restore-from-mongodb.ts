...
async function restoreFromBackup(backupId?: string): Promise<void> {
    ...
    for (const table of backup.metadata.tables) {
        const size = table.sizeBytes ? `(${(table.sizeBytes / 1024).toFixed(1)} KB)` : "";
        console.log(`   - ${table.name}: ${table.recordCount.toLocaleString()} records ${size}`);
    }
    console.log("");
    ...
    const answer1 = await askQuestion("Type 'YES' in ALL CAPS to proceed with restore: ");
        ...
        try {
            ...
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                ...
                try {
                    ...
                    const result = await client.mutation(api.restore.restoreTableBatch as any, {
                        tableName,
                        records: batch,
                        isFirstBatch,
                    });
                    ...
                    if ((batchIndex + 1) % 5 === 0 || batchIndex === batches.length - 1) {
                        console.log(`   Progress: ${batchIndex + 1}/${batches.length} batches (${totalInserted} records)`);
                    }
                } catch (batchError) {
            ...
            errors.push(`${tableName}: ${error instanceof Error ? error.message : String(error)}`);
            errorCount++;
        }
    }
